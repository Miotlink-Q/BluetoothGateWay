...
package com.miot.android.data.d.utils;
...
import com.miot.android.data.d.file.MultiCardFilePath;
...
//            LogUtils.d("内存不足");
     ...
     * @return the bitmap
     */
    public static Bitmap scaleImageByPath(String pathString, Context context) {
        Bitmap retBm = null;
        if (Util.isEmpty(pathString)) {
            return retBm;
        }
        // 路径文件不存在
        {
        ...
        try {
            ...
            Bitmap srcBitmap;
            {
                ...
                BitmapFactory.decodeFile(pathString, bitmapOptions);
                bitmapOptions.inJustDecodeBounds = false;
                int inSampleSize = bitmapOptions.outWidth / ScreenUtil.screenWidth;
                if (inSampleSize < 1)
                    inSampleSize = 1;
                bitmapOptions.inSampleSize = inSampleSize;
                ...
                srcBitmap = BitmapFactory.decodeStream(is, null, bitmapOptions);
                if (srcBitmap == null)
                    return retBm;
                {
                    retBm = srcBitmap;
            ...
            // if (inSampleSize>1)
            // retBm = scaleImage(srcBitmap,context);
        } catch (Exception e) {
            return retBm;
        }
