...
import com.miot.android.data.d.utils.IOUtils;
...
public class DbUtils {
	...
	public void save(Object entity, Class<?> classObj) throws DbException {
		if (!entity.getClass().isAssignableFrom(classObj)) {
			throw new DbException("不属于继承关系");
		}
		try {
			beginTransaction();
			createTableIfNotExist(entity.getClass());
	...
	public void saveAll(List<?> entities, Class<?> classObj) throws DbException {
		if (entities == null || entities.size() == 0)
			return;
		if (!entities.get(0).getClass().isAssignableFrom(classObj)) {
			throw new DbException("不属于继承关系");
		}
		try {
			beginTransaction();
		...
		if (cursor != null) {
			try {
				if (cursor.moveToNext()) {
					T entity = CursorUtils.getEntity(this, cursor, entityType,
							seq);
...
//				IOUtils.closeQuietly(cursor);
	...
	@SuppressWarnings("unchecked")
	public <T> List<T> findAll(String sql, Class<?> entityType)
			throws DbException {
		if (!tableIsExist(entityType))
			return null;
		long seq = CursorUtils.FindCacheSequence.getSeq();
...
//				IOUtils.closeQuietly(cursor);
	...
	public boolean saveBindingIdWithoutTransaction(Object entity)
			throws DbException {
